...
class Multiplayer:
    def __init__(self):
        ...
        self.server_ADDR = ('35.176.207.55', 5555)

        self.running = True

    def ServerContact(self):
        self.GamePlay.connect((self.server_ADDR))
        print(self.GamePlay.recv(2048).decode())
        self.GamePlay.send(input('Enter Code : ').encode())
        GamePort = int(self.GamePlay.recv(2048).decode())
        print(GamePort)
        self.GamePlay.close()
    ...
    def Manager(self):
        while self.running:
            dataReceived = self.GamePlay.recv(2048)
            try:
