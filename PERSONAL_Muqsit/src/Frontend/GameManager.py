...
sys.path.append("..")
from Backend import Library_DRAFT
...
class ThreeXThreeBoardScene():
    ...
    def startGame(self):
        ...
        # win, board = newGameInstance.start()
        while self.BoxesFilled < 9:
            try:
                thread = ThreadPool(processes=1)
                threadReturn = thread.apply_async(Library_DRAFT.LocalPlayer.GamePlay, (Library_DRAFT.LocalPlayer(),))
                x, y = threadReturn.get()
