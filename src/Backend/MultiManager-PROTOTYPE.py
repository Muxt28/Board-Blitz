...
class GmaeManager:
    ...
    def Initialise(self):
        ...
        self.clientsConnected[1].send('2'.emcode())
    ...
    def GamePlay(self):
        ...
        while (self.running or self.BoxesFilled < 9) and (win == False):
            ...
            if self.BoxesFilled % 2 == 0:
                self.GameSocket[0].send('*[ Your Turn ]*')
                self.GameSocket[1].send('*[ Player 1 Turn ]*')
                self.currentPlayer = 'X'
            else:
                self.GameSocket[1].send('*[ Your Turn ]*')
                self.GameSocket[0].send('*[ Player 2 Turn ]*')
